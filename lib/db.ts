...
export async function loadHeroes(walletAddress: string): Promise<Hero[]> {
  ...
  return data.map((h: any) => ({
    ...
    ability: h.power,
    gender: h.gender,
    health: Number(h.attack ?? 10),
    power: Number(h.attack ?? 10),
    defense: Number(h.defense ?? 10),
    luck: Number(h.speed ?? 10),
    xp: h.xp,
    level: h.level,
...
export async function saveItem(item: Item, walletAddress: string) {
  const { data, error } = await supabase()
    ...
    .insert({
      ...
      slot: item.slot,
      rarity: item.rarity,
      bonus_atk: item.bonusPWR,
      bonus_def: item.bonusDEF,
      bonus_spd: item.bonusLCK,
...
export async function loadItems(walletAddress: string): Promise<Item[]> {
  ...
  return data.map((row: any) => ({
    ...
    slot: row.slot,
    rarity: row.rarity,
    bonusPWR: row.bonus_atk,
    bonusDEF: row.bonus_def,
    bonusLCK: row.bonus_spd,
